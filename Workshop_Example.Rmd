...
PhenoCam data. This code was adopted from code by Bijan Seyednasrollah
and Koen Hufkens.
...
1001, 1002). The packages we will use today allow us to interact with
...
retrive the latest metadata for all sites of the PhenoCam network.
...
with a co-located flux tower, we can filter the metadata using the
...
Forest. This site has co-located PhenoCam and eddy covariance (EC) flux
...
then merge it with the flux data. After merging, we will look at the
seasonal patterns by plotting the 90th percentile Green Chromatic
Coordinate (GCC) and Gross Primary Productivity (GPP) over time,
followed by an interactive scatter plot to investigate the relationship
between GCC and GPP.
...
dates. First, let's download the BERMS Southern Old Black Spruce Site
data in Prince Albert National Park using this package.
...
Here we use the phenophases() function to calcualte the phenophase transition dates at the Bartlett Experimental Forest to connect those phases to the flux data, namely GPP.
