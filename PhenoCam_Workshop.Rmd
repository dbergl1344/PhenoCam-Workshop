...
# How can we connect flux data to PhenoCam data?

### We can filter using attributes

Alternatively, we may want to only include PhenoCam sites with certain
attributes in our datasets. For example, we may be interested only in
sites with a co-located flux tower. For this, we'd want to filter for
those with a flux tower using the `flux_sitenames` attribute in the
metadata.
...
What if we are interested PhenoCam data and Flux data that is co-located
within a specific vegetation type? We can do that! We could further
identify which of those Phenocams with a flux tower and in deciduous
broadleaf forests (`primary_veg_type=='DB'`).
...
```{r}
# Assuming we already have Bartlett PhenoCam data loaded and processed
# Bartlett PhenoCam data already exists in 'merged_fluxPhenodata'

# Step 1: Download and assign Bartlett PhenoCam data (if not already downloaded)
# If Bartlett data isn't available, use the download function below
...
# Step 2: Detect outliers and smooth the time series
dfBart <- detect_outliers(dfBart)
dfBart <- smooth_ts(dfBart)

# Step 3: Calculate phenophases for the Bartlett data
...
# Step 4: Plot GPP and overlay phenophases (transitions at 10%, 25%, 50%) on top of GPP

# Plot GPP over time
# Assuming GPP data is already in 'merged_fluxPhenodata'
# Convert the date column to Date format if it's not already
merged_fluxPhenodata$date <- as.Date(merged_fluxPhenodata$date)
...
plot(as.Date(merged_fluxPhenodata$date),
     ...
     col = "cornflowerblue",
     main = "GPP with Phenophase Transitions at Bartlett")

# Add vertical lines for rising and falling phenophase transitions from dfBart

# Convert numeric transition dates to Date format (assuming origin "1970-01-01")
rising_transition_50 <- as.Date(phenology_dates$rising$transition_50, origin = "1970-01-01")
falling_transition_50 <- as.Date(phenology_dates$falling$transition_50, origin = "1970-01-01")

rising_transition_10 <- as.Date(phenology_dates$rising$transition_10, origin = "1970-01-01")
falling_transition_10 <- as.Date(phenology_dates$falling$transition_10, origin = "1970-01-01")

rising_transition_25 <- as.Date(phenology_dates$rising$transition_25, origin = "1970-01-01")
falling_transition_25 <- as.Date(phenology_dates$falling$transition_25, origin = "1970-01-01")

# Rising (spring green-up) transition 50%
abline(v = rising_transition_50, col = "forestgreen", lwd = 1)

# Falling (autumn senescence) transition 50%
