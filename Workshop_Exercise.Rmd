...
In this part of the workshop, you will select your own site to explore. First, we need to install and load the appropriate libraries.
...
#To view all of the sites and select which site you might be interested in, you can use the view() function. From this list of sites in the dataframe
# Create a new DataFrame with the site information and vegetation type for a site that you may be interested in
selected_columnsdf <- data.frame(Site = mydf$site, Vegetation_Type = mydf$primary_veg_type)

# View the new DataFrame that has all of the sites listed with the vegetation type
...
At this point, choose two vegetation types that you might be interested in exploring and plot their Mean Annual Precipitation and Mean Annual Temperature. See the table below for the vegetation abbreviation. To do this we'd first want to remove the sites where there is not data and then 
plot the data that is complete. 
...
or by using the `get_rois()` function. This function connects directly to the PhenoCam website and can easily be overloaded. For the purposes of this workshop exercise, we will use the roi dataframe that we have already downloaded.  
...
Let's use this measure to look at changes in GCC over time at your site. Looking 
back at the available data, we have several options for GCC. `gcc90` is the 90th 
quantile of GCC in the pixels across the ROI.
